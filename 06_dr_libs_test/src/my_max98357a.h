...
#define I2S_DATA_OUT_PIN    5   // DIN - 数据输入（I2S）
#define I2S_BCK_PIN         18   // BCK - 时钟信号（I2S）
#define I2S_LRCK_PIN        19   // LRCLK - 左右声道时钟信号（I2S）
...
typedef void (*I2S_SEND_CALLBACK)(void);
I2S_SEND_CALLBACK i2s_send_callback = NULL;
...
void i2s_task2(void *pvParameter) 
{
  ...
  while (1)
  {
    if (getCurrentLength(&audioStreamBuf)  > 256) 
    {

      // 批量读取数据
      size_t bytesRead = readBatchCircularBuffer(&audioStreamBuf, readData, 512);
      if (bytesRead < 0) {
        continue;
      } 
      ...
      convertAndApplyGain(readData, pcmData, bytesRead, 0.01 * gain);

      i2s_write(I2S_NUM, pcmData, bytesRead, &bytes_written, portMAX_DELAY);
...
void max98357Setup(I2S_SEND_CALLBACK cb) {
  i2s_send_callback = cb;
  ...
  // 创建 I2S 播放任务
  // xTaskCreate(i2s_task2, "I2S Task", 14240, NULL, 4, NULL);
